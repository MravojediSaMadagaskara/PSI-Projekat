...
<html>
	...
	<head>
		...
		<script src = "<?php echo base_url();?>js/uredjivanje.js"></script>
	</head>
		...
		<div id = "sredina">
			<div id = "sredinalevo">
				...
				<form>
					...
					<br>
					<select id="prijavljenapitanja" name="prijavljenapitanja" form="" size="10" onclick="selektovanje('Prijavljena')">
						<?php
							for ($i = 1; $i <= $brojprijavljenih; $i++){
								echo '<option value="'.$i.'"';
								if ($i==1 && $prijavljeno=='Da') echo 'selected';
								echo'>Pitanje '.$i.'</option>';
							}
						?>
					</select>
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					<select id="ostalapitanja" name="ostalapitanja" form="" size="10" onclick="selektovanje('Ostala')">
						<?php
							for ($i = 1; $i <= $brojostalih; $i++){
								echo '<option value="'.$i.'"';
								if ($i==1 && $prijavljeno=='Ne') echo 'selected';
								echo'>Pitanje '.$i.'</option>';
							}
						?>
					</select>
			...
			<div id = "sredinadesno">
				...
				<form>
					...
					<br><br>
					<button type="button" onclick="ostavi()" <?php if ($prijavljeno=='Ne') echo 'style="display:none"' ?>>Ostavi</button>
